...
/*   Updated: 2018/02/02 03:59:19 by hbouchet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_v			rotation(t_v n, t_v r, float t)
{
	...
	c = cos(t / 360 * 2 * M_PI);
	s = sin(t / 360 * 2 * M_PI);
	v.x = (r.x * r.x * (1 - c) + c) * n.x + (r.x * r.y * (1 - c) - r.x * s) * n.y
		+ (n.x * n.z * (1 - c) + n.y * s) * n.z;
	v.y = (r.x * r.y * (1 - c) + r.z * s) * n.x + (r.y * r.y * (1 - c) + c) * n.y
		+ (r.y * r.z * (1 - c) - r.x * s) * n.z;
	v.z = (r.x * r.z * (1 - c) - r.y * s) * n.x + (r.y * r.z * (1 - c) + r.x * s) * n.y + (r.z * r.z * (1 - c) + c) * n.z;
	return (v);
}

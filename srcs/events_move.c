...
/*   Updated: 2018/02/15 08:58:55 by hbouchet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (env->curr_obj == -1)
		cam->pos_cam = vect_add(cam->pos_cam,
...
static int	events_move2(t_env *env, unsigned int sym, t_cam *cam, t_obj *obj)
{
	if (env->curr_obj == -1 && sym == SDLK_a)
	{
		...
		cam->pos_cam = vect_add(cam->pos_cam, vect_mult(cam->v3cam, 50.0));
	}
...
int			events_move(t_env *env, unsigned int sym, t_cam *cam, t_obj *obj)
{
	if (env->curr_obj == -1 && (sym == SDLK_UP || sym == SDLK_q))
		cam->pos_cam = vect_add(cam->pos_cam, vect_mult(cam->vcam, 50.0));
		...
		cam->pos_cam = vect_add(cam->pos_cam, vect_mult(cam->v2cam, 50.0));
	else if (env->curr_obj >= -1 && sym == SDLK_w)

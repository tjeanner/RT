...
t_color		next_rays(t_objs *objs, t_lums *lums, t_ray *line, unsigned int d)
{
	...
	c[0] = 0.0;
	k = 0.;
	if (line->obj >= 0 && objs->obj[line->obj].transp > 0.0)
	{
		k = line->n1 / line->n2;
		...
		r[1] = r[1] * r[1];
		r[2] = (r[0] + r[1]) / 2.0;
		col = mult_color(get_refract(objs, lums, line, d), objs->obj[line->obj].transp * r[2]);
	}
	if (r[2] < 1.0)
		col = add_color(col, mult_color(
							get_reflect(objs, lums, line, d), objs->obj[line->obj].transp * (1.0 - r[2])));
	return (col);
}
*/

...
/*   Updated: 2018/01/27 04:15:32 by hbouchet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			init_ray(t_env *env, float x, float y)
{
	...
	env->cams[0].v3cam = vect_mult(vect_prod(env->cams[0].vcam, env->cams[0].v2cam), -1);
	cam_2_center_screen = vect_mult(env->cams[0].vcam, DIST);
	center_screen_2_pix = vect_add(vect_mult(env->cams[0].v3cam, (x - WIN_X / 2) / vect_norm(env->cams[0].v3cam)),

...
int			get_dist_cone(t_ray *init_rays, t_obj obj)
{
	...
	vs = vect_scal_prod(init_rays->r2, s);
	...
	ra0 = vect_prod(vect_prod(s, vect_add(init_rays->r, vect_mult(obj.norm, -1.0))), s);
...
t_color		get_col(t_env *env, float x, float y)
{
	...
	//	res = vect_scal_prod(env->pos_lum, vect_mult(pos_collision, -1.0));
		...
		else if (env->objs[ob].type == 's')
		{
			...
			env->init_rays.v1 = 0;
			env->init_rays.v2 = 0;
			if (get_dist_sphere(&env->init_rays, env->objs[ob]) == 1 && (env->init_rays.v1 > 1 || env->init_rays.v2 > 1))
				set_black(&col);
			init_ray(env, x, y);
		}
	}

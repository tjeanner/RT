...
/*   Updated: 2018/01/25 01:40:44 by tjeanner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_dist_cone(t_env *env, t_obj obj)
{
	...
	vs = vect_scal_prod(env->init_rays.r2, s);
	hh = vect_norm(vect_add(obj.o, vect_mult(obj.norm, -1.0)));
	hh = obj.radius * obj.radius / (hh * hh);
	h = vect_scal_prod(s, vect_add(env->init_rays.r2, vect_mult(obj.norm, -1.0)));
	va = vect_prod(vect_prod(s, env->init_rays.r2), s);
	w = 50.0 - vect_scal_prod(s, vect_add(env->init_rays.r, vect_mult(obj.norm, -1.0)));
	ra0 = vect_prod(vect_prod(s, vect_add(env->init_rays.r, vect_mult(obj.norm, -1.0))), s);
	...
	{
		env->init_rays.v1 = (-b + sqrt(tmp)) / (2 * a);//if tmp == 0
		env->init_rays.v2 = (-b - sqrt(tmp)) / (2 * a);//env->init_rays.v2 will be the same as env->init_rays.v1
	}
	return (1);
}
...
int			get_dist_tube(t_env *env, t_obj obj)
{
	...
	s = vect_add(obj.o, vect_mult(obj.norm, -1.0));
	s = vect_mult(s, 1.0 / vect_norm(s));
	va = vect_prod(vect_prod(s, env->init_rays.r2), s);//Va
	ra0 = vect_prod(vect_prod(s, vect_add(env->init_rays.r, vect_mult(obj.norm, -1.0))), s);//Ra0
	...
	{
		env->init_rays.v1 = (-b + sqrt(tmp)) / (2 * a);
		env->init_rays.v2 = (-b - sqrt(tmp)) / (2 * a);
	}
	return (1);
}
...
{
	env->init_rays.v1 = -1;
	env->init_rays.v2 = vect_scal_prod(obj.norm, vect_add(obj.o, vect_mult(env->init_rays.r, -1))) / vect_scal_prod(obj.norm, env->init_rays.r2);
	return (1);
}

/*
**get_dist: a function that will return 0, 1 or 2 distances between a sphere and
**the pixel. the distance(s) is written in env->init_rays.v1 and env->init_rays.v2
*/
...
int			get_dist_sphere(t_env *env, t_obj obj)
{
	...
	pixel_2_sphere_center = vect_add(env->init_rays.r, vect_mult(obj.o, -1));
	a = vect_scal_prod(env->init_rays.r2, env->init_rays.r2);
	b = 2 * vect_scal_prod(env->init_rays.r2, pixel_2_sphere_center);
...
//	env->init_rays.r = vect_add(env->cam.pos_cam, cam_2_pixel);
...
t_color		get_col(t_env *env, float x, float y)
{
	...
	//	res = vect_scal_prod(env->pos_lum, vect_mult(pos_collision, -1.0));
		...
		else if (env->objs[ob].type == 's')
		{
			...
			col.c.g = env->objs[ob].col.c.g * res / 1;
			col.c.r = env->objs[ob].col.c.r * res / 1;
			env->init_rays.r2 = lum_2_collision;
			env->init_rays.r = pos_collision;
			env->init_rays.v1 = 0;
			env->init_rays.v2 = 0;
			if (get_dist_sphere(env, env->objs[ob]) == 1 && (env->init_rays.v1 > 1 || env->init_rays.v2 > 1))
				set_black(&col);
			init_ray(env, x, y);
		}
	}

...
/*   Updated: 2018/01/28 10:03:34 by tjeanner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_dist_tube(t_ray *init_rays, t_obj obj)
{
	...
	{
		init_rays->v1 = (-b + sqrt(tmp)) / (2.0 * a);
		init_rays->v2 = (-b - sqrt(tmp)) / (2.0 * a);
	}
	return (1);
}
...
{
	init_rays->v1 = -1;
	init_rays->v2 = vect_scal_prod(obj.norm, vect_add(obj.o, vect_mult(init_rays->r, -1.0))) / vect_scal_prod(obj.norm, init_rays->r2);
	return (1);
}
